...
- Built: `manifest.webmanifest`, `sw.js` (version-stamped cache, relative scope, old caches deleted on activate), `js/updates.js` (reload at the run boundary, never with a panel open), `js/wakelock.js`, `js/memory.js`, `tools/make-icons.mjs` and the four committed PNGs, and the full README. Extended `tools/set-version.mjs` to stamp the manifest and the cache name and to regenerate the precache list from what is actually on disk, so it cannot go stale as modules are added.
- Decided: the icon is generated by importing the app's own `palette.js`, `world.js` ramp and `shapes.js` fish builder in Node and writing the PNG bytes directly with `node:zlib`. No canvas, no dependency, and the icon is literally drawn from the same data as the app. Each size is composed on its own whole-pixel grid (180 = 60x3, 192 = 64x3, 512 = 64x8) so no icon is ever resampled off the pixel grid.
- Decided: the never-sleep video fallback is recorded at runtime from a 2x2 canvas via `MediaRecorder` rather than embedded as a base64 asset — the no-asset-files rule holds, and there is nothing to go stale. It is best-effort and only used when the Wake Lock API is missing or has already refused.
- Decided: the atlas survives a freeze and the water strip does not. The strip is the large regenerable buffer (1.5-3.4 MB); the atlas is the expensive one to rebuild and the cheap one to hold.

### Four defects found and fixed

1. **Far-layer creatures spawned in plain view.** `spawnPos` worked out its off-screen positions in layer-1 world coordinates, but a creature on the 0.5 haze layer sits somewhere else entirely. Hammerheads and sixgills were appearing mid-frame. The spawner now converts the visible band into each species' own layer coordinates first.
2. **A full pool recycled whatever was oldest, visible or not** — so at the cap an animal could simply vanish on screen. `makePool.acquire()` now prefers the oldest entity that is off screen, and only falls back to the oldest overall if every slot is visible.
3. **The service worker's first install armed a spurious reload.** `clients.claim()` fires `controllerchange` on a page that never had a controller; that is an install, not an update. Guarded on whether a controller existed at registration.
4. **`freeze` and `resume` were listened for on `window`.** They fire at `document` and do not bubble, so the release-and-rebuild path would never have run on a real freeze. Corrected in `memory.js`, `wakelock.js` and `save.js`.

Five provider strings also carried a literal `\n` into the diagnostics blob instead of a newline.

### The Definition of done, walked

Verified here (headless Chromium via the CDP driver, unless noted):

- Each zone identifiable at a glance; zone 4 empty and zone 6 dark on purpose; three size bands at once; the Huge animals read as huge — checked by screenshot at every zone.
- Every animal moves like the thing it is based on, no two behaviour types alike — checked in the `?debug=behaviours` proving ground.
- One vignette per zone visit, finished before the camera leaves, no back-to-back repeats, two consecutive runs different — two 95 s runs shared none of their four scenes.
- **Nothing pops in or out on screen** — 700 samples at 10 Hz across a full run at 6x speed: zero spawns and zero retirements while visible (this is the check that found defects 1 and 2).
- Glossary generated from the registry, seen state survives closing, locked entries are silhouettes.
- Every setting applies immediately and survives a restart; a URL parameter overrides visibly without being saved.
- Runs normally with `localStorage` unavailable, and a corrupt or future-version save is backed up rather than crashed on — five-case suite, all clean.
- Title loads the app behind it, fades rather than cuts, and resumes the last depth within five minutes.
- Version matches in all four places: `js/version.js`, the manifest, the service worker cache name and the README.
- A new worker reloads at the end of a run and **not** while a panel is open — held across a full run with the glossary open, reloaded within 2 s of closing it.
- Works offline after the first load — the origin server was killed outright and a reload still ran the full aquarium from cache (67 entries).
- Portrait composition survives rotation — landscape gives `rotate(90deg)`, a still-portrait internal canvas and coverage on both axes.
- The screen-awake layers: the API is acquired and re-acquired, `?awake=0` and the settings row both release it, and freeze/resume drops and re-takes it.
